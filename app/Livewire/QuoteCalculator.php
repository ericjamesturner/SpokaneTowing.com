...
class QuoteCalculator extends Component
{
    ...
    protected $rules = [
        'fromAddress' => 'required|min:5',
        'toAddress' => 'required|min:5',
    ];
    ...
    public function calculateQuote()
    {
        ...
        try {
            ...
            if ($result) {
                ...
                $this->dispatch('quote-calculated');
            }
        } catch (\Exception $e) {
    ...
    private function calculateDistance($from, $to)
    {
        ...
        if (empty($apiKey)) {
            ...
            // This is temporary until Google Maps API key is provided
            return [
    ...
    public function render()
    {
        return view('livewire.quote-calculator', [
            'googleMapsApiKey' => config('towing.google_maps.api_key')
        ]);
    }
}